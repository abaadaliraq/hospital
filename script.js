(function () {
  ...
  var openableImages = Array.prototype.slice.call(document.querySelectorAll(".image-shell img")).filter(function (img) {
    var parent = img.parentElement;
    var alt = img.getAttribute("alt") || "";
    return parent && !parent.classList.contains("logo-shell") && !parent.classList.contains("logo-large") && alt.trim() !== "";
  });
  ...
  if (openableImages.length) {
    ...
    closeLightbox.setAttribute("aria-label", "إغلاق الصورة");
    ...
    openableImages.forEach(function (img) {
      var shell = img.parentElement;
      shell.classList.add("is-openable");
      shell.setAttribute("role", "button");
      shell.setAttribute("tabindex", "0");
      shell.setAttribute("aria-label", "فتح الصورة");
    ...
    lightbox.addEventListener("click", function (event) {
      if (event.target === lightbox) hideImage();
    });
